 ...
 *
 * ## `ACTION_DATE_CHANGED` — the ordinary midnight rollover
 *
 * `TIME_SET` and `TIMEZONE_CHANGED` only fire for a *manual* or *network*
 * clock change. An ordinary midnight — the device's date advancing by one day
 * with no clock edit at all — sends neither. If the app's process happens to
 * still be alive at midnight (a foreground timer, a habit sheet left open, an
 * OEM that keeps the process warm), nothing was telling it the day had turned
 * over.
 *
 * That is the "alarm doesn't ring once the day is different, and it needs a
 * reset" report: [AlarmRingState]'s "already answered today" gate is keyed on
 * `Dates.todayKey()`, which is correct the instant it's *read* — but a stale
 * in-memory assumption elsewhere in a long-lived process, or a ring that was
 * being delivered in the last seconds before midnight, can leave the new day
 * starting with state that still describes the old one. `DATE_CHANGED` is
 * handled exactly like a clock change here: it clears the ring gate and
 * re-verifies every habit alarm is armed, so the fresh day reliably starts
 * with its own dialog rather than inheriting yesterday's answered state.
 *
 * ## These five are all system broadcasts
 *
 * `ACTION_BOOT_COMPLETED`, `ACTION_MY_PACKAGE_REPLACED`, `ACTION_TIME_CHANGED`,
 * `ACTION_TIMEZONE_CHANGED` and `ACTION_DATE_CHANGED` are system/protected
 * actions, so the manifest's `exported="false"` is correct and does not stop
 * them arriving: a non-exported *manifest* receiver still receives system
 * broadcasts, and a *dynamic* registration is delivered from anywhere in the
 * system. Nothing here is reachable by another app.
 */
class BootReceiver : BroadcastReceiver() {
    ...
    private fun rescheduleAll(context: Context, intent: Intent) {
        ...
        // A clock/timezone change — or an ordinary midnight rollover —
        // invalidates today's "already delivered" acknowledgement, because the
        // (day, time) it names was resolved under the OLD day/clock. See the
        // class note for why this is safe, and why DATE_CHANGED belongs here
        // too: it is the one day-boundary event TIME_SET/TIMEZONE_CHANGED
        // never cover.
        val isClockChange = action == ACTION_TIME_CHANGED ||
            action == ACTION_TIMEZONE_CHANGED ||
            action == ACTION_DATE_CHANGED
        if (isClockChange) {
    ...
    private companion object {
        ...
        /**
         * `android.intent.action.DATE_CHANGED` — the device's date advanced,
         * normally the ordinary midnight rollover (also sent after a manual
         * date edit). Same literal-over-constant reasoning as
         * [ACTION_TIME_CHANGED]. Unlike `TIME_SET`, this fires with NO clock
         * edit at all, which is exactly the gap that let a long-lived process
         * cross midnight without ever being told the day had changed.
         */
